...
def set_seed(args):
    """
    Sets seed for reproducible evaluation.
    :param args: args.
    :return: none.
    """
...
def multi_class_roc_auc_score(args, ground_truths, prob_dist, average='macro', multi_class='ovr', best_model_id=0):
    ...
    try:
        ...
        if args.encoding_cat == 'int':
            ...
            return sklearn.metrics.roc_auc_score(ground_truths_existing_column, prob_dist_existing_column,
                                                 average=average,
...
def write_measures(args, _measures):
    ...
    if args.mode == 2:
        values.append(args.xai)
        values.append(args.removed_events_relevance)
        values.append(args.removed_events_num)
...
def get_model_dir(args, model_id=0, scaler=False):
    """
    ...
    if scaler:
        model_dir = "%s%s_%s_%s_%s" % (args.model_dir, args.task,
                                       args.data_set[0:len(args.data_set) - 4], "scaler", args.model_id)
    else:
        model_dir = "%s%s_%s_%s_%s" % (args.model_dir, args.task,
...
def load_nap_model(args, model_id=0, scaler=False):
    """
    ...
    if scaler:
        model_dir = get_model_dir(args, model_id, scaler)
    else:
        model_dir = get_model_dir(args, model_id, scaler)
    if args.classifier == "LSTM":
